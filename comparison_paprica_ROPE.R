#This script is to compare taxonomy between ROPE and paprica at 16S level.
#The output file will contain ROPE taxonomy file on the left and paprica taxonomy file on the right
#author: Avishek Dutta (avdutta@ucsd.edu)

#reading paprica taxon map file

pap_tax = read.csv("ROPE_16S.bacteria.taxon_map.csv", header=TRUE)
...
unqe_edge = read.csv ("ROPE_16S.bacteria.seq_edge_map.csv", header= TRUE)
...
ROPE_tax = read.csv("taxa_map_ROPE_unique.csv", header=TRUE)
...
write.csv (merge3, "ROPE_paprica_comaprison_unique_16S.csv")